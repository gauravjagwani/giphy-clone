...
const FilterGif = ({ alignLeft = false, showTrending = false }) => {
  ...
  return (
    <div
      className={`flex gap-3 py-4 ${alignLeft ? "" : "justify-end"} ${
        showTrending
          ? "justify-between flex-co sm:flex-row sm:items-center"
          : ""
      }`}
    >
      {showTrending && (
        <span className="flex gap-3 items-center mb-2 mt-2">
          {showTrending && (
            <HiMiniArrowTrendingUp size={25} className="text-teal-300" />
          )}
          <span className="font-semibold text-gray-400">Trending</span>
        </span>
      )}
      <div className="flex gap-3 min-w-80 rounded-full bg-gray-800">
        {filters.map((f) => {
          return (
            <span
